...
-- Extra which-key mappings (which-key v3 specs via wk_add):
-- wk_add({
...
M.config = {
  -- Merge into conform.nvim setup. Shipped example enables format-on-save with LSP
  -- fallback; add formatters_by_ft when you want non-LSP formatters (still opt-in).
  conform = {
    format_on_save = {
      timeout_ms = 500,
      lsp_fallback = true,
  ...
  --   },
  -- },
  -- nvim-treesitter (main): opt-in parsers only (no defaults).
  -- treesitter = {
