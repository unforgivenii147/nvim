local which_key = require("utils").safe_require("which-key")
...
if config.which_key then
	which_key_config = vim.tbl_deep_extend("force", which_key_config, config.which_key)
end
...
if is_plugin_installed("true-zen.nvim") and not disable_plugins.truezen then
...
which_key.add({
	{
		"<leader>r",
		function()
			local use_conform = is_plugin_installed("conform.nvim") and not disable_plugins.conform
			if use_conform then
...
if is_plugin_installed("lspsaga.nvim") and not disable_plugins.lspsaga then
	...
	table.insert(
		lsp_maps,
		{ "<leader>ld", ":Lspsaga show_line_diagnostics<CR>", desc = "Show Current Line Diagnostics" }
	)
	...
	table.insert(
		lsp_maps,
		{ "<leader>ld", ":lua vim.diagnostic.open_float()<CR>", desc = "Show Current Line Diagnostics" }
	)
...
if is_plugin_installed("nvim-dap") or is_plugin_installed("nvim-dap-ui") then
	...
	if is_plugin_installed("nvim-dap") then
		...
		table.insert(
			debug_maps,
			{ "<leader>dl", ':lua require("dap").run_last()<CR>', desc = "Run Last Debugging Config" }
		)
