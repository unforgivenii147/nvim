local lualine_setup = require("utils").safe_require("lualine")
		...
		lualine_x = {
			...
			},
			{
				...
				color = {
					...
					fg = (vim.opt.termguicolors:get() and vim.fn.synIDattr(
						vim.fn.synIDtrans(vim.fn.hlID("DiffChange")),
						"fg"
					)),
				},
			},
...
if config.lualine then
	lualine_config = vim.tbl_deep_extend("force", lualine_config, config.lualine)
end
